...
gcc_arm_install_folder="gcc-arm-none-eabi-${GCC_ARM_VERSION_AM263X }"
gcc_arm_download_file="gcc-arm-none-eabi-${GCC_ARM_VERSION_AM263X}-x86_64-linux.tar.bz2"
...
clang_url_folder="${CGT_TI_ARM_CLANG_VERSION_AM263X}.${CGT_TI_ARM_CLANG_VERSION_SUFFIX}"
clang_install_folder="ti-cgt-armllvm_${CGT_TI_ARM_CLANG_VERSION_AM263X}.${CGT_TI_ARM_CLANG_VERSION_SUFFIX}"
clang_install_file="ti_cgt_armllvm_${CGT_TI_ARM_CLANG_VERSION_AM263X}.${CGT_TI_ARM_CLANG_VERSION_SUFFIX}_linux-x64_installer.bin"
...
if [ "${OS}" = "Windows_NT" ]; then
    ...
    if [ "$skip_ccs" == "false" ]; then
        install_ccs     ${CCS_VERSION_AM263X} ${install_dir}
    fi
    install_clang   ${CGT_TI_ARM_CLANG_VERSION_AM263X} ${clang_url_folder} ${clang_install_folder} ${clang_install_file} ${install_dir}
    install_gcc_arm ${GCC_ARM_VERSION_AM263X} ${gcc_arm_install_folder} ${gcc_arm_download_file} ${install_dir} ${GCC_ARM_VERSION_FOLDER}
    install_syscfg  ${SYSCFG_VERSION_AM263X} ${install_dir}
    ...
    ccs_discover_tools  ${CCS_VERSION_AM263X} ${install_dir}
fi
